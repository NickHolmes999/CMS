...
# Remove rows with missing emails or 'Track Selection'
df = df.dropna(subset=['Email', 'Track Selection'])

# Convert relevant columns to strings and strip whitespace
df['Email'] = df['Email'].astype(str).str.strip()
df['Track Selection'] = df['Track Selection'].astype(str).str.strip()
df['Season Selection'] = df['Season Selection'].astype(str).str.strip()
...
# Debug: Print unique 'Season Selection' values before standardization
print("Unique 'Season Selection' values before standardization:")
print(df['Season Selection'].unique())
...
# Apply the function to standardize 'Season Selection'
df['Season Selection'] = df['Season Selection'].apply(standardize_season)

# Debug: Print unique seasons after standardization
print("Unique seasons after standardization:")
print(df['Season Selection'].unique())

# Extract years from 'Season Selection'
years_in_data = df['Season Selection'].str.extract(r'(\d{4})')[0].dropna().astype(int).unique()
...
# Handle missing 'Season Selection' values (NaN)
df['Season Selection'] = df['Season Selection'].fillna('Unknown Season')

# Check for any 'Unknown Season' values in 'Season Selection' after conversion
unknown_seasons = df[df['Season Selection'] == 'Unknown Season']
if not unknown_seasons.empty:
    print("Warning: The following records have 'Season Selection' values not recognized:")
    print(unknown_seasons[['Email', 'Season Selection']])
...
# df = df[df['Season Selection'] != 'Unknown Season']
...
df = df.groupby(['Email', 'Season Selection'], as_index=False).apply(get_status).reset_index(drop=True)
...
def build_full_track_name(row):
    class_taken = str(row['Track Selection']).strip()
...
def calculate_season_statistics(df, track):
    ...
    for season in season_order:
        season_df = track_df[track_df['Season Selection'] == season]
    ...
    for user in initial_users:
        ...
        for _, initial_course in initial_course_instances.iterrows():
            ...
            if pd.isnull(initial_season):
                continue  # Skip if 'Season Selection' is missing

            initial_season_idx = season_order.index(initial_season)

            later_courses = user_courses[
                user_courses['Season Selection'].apply(lambda x: season_order.index(x) if x in season_order else -1) > initial_season_idx
            ]
