...
# def make_update(responseId,token, surveyId, EDObj):
    ...
    print("Body: {0}".format(response.content))
    if "200" not in str(response.status_code):
        write_log("errors.txt", str(response.content))
        return responseId
    else:
        return 0    
...
class SetSurveyId(argparse.Action):
    survey_choices = {'Awareness_Legacy' : 'SV_9EtYLCBZb9rJ8Ds', 
                      ...
                      'Product': 'SV_9WtquKQbPFMcFcq',
                      'Event_Reg': 'SV_bKrHfWmJITfCZZY'}
...
def main():
    ...
    action.add_argument('--reapeat_learners', help= "search for reapeat learners and make the updates", 
                        action = 'store_true')
    ...
    if args.bulkUpdateFromCSV != None:
